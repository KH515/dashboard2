...
const inp: React.CSSProperties = { width:"100%", padding:"9px 12px", background:"#f0f0f0", border:"1px solid #2a2a2a", borderRadius:"9px", color:"white", outline:"none", fontFamily:"Cairo,sans-serif", fontSize:"13px", textAlign:"right", boxSizing:"border-box" }
const lbl: React.CSSProperties = { fontSize:"11px", color:"#555", marginBottom:"5px", display:"block" }
...
function ImgField({ value, onChange, label }: { value:string; onChange:(v:string)=>void; label:string }) {
  ...
  return (
    <div style={{ marginBottom:"12px" }}>
      ...
      <div onClick={()=>ref.current?.click()} style={{ width:"100%", borderRadius:"10px", border:"1px dashed #333", background:"#f0f0f0", cursor:"pointer", overflow:"hidden", minHeight:"52px", display:"flex", alignItems:"center", justifyContent:"center" }}>
        ...
        {filtered.slice(0,30).map(p => {
          ...
          return (
            <div key={p.id} onClick={()=>onChange(sel?selected.filter(id=>id!==p.id):[...selected,p.id])}
              ...
              <div style={{ width:34, height:34, borderRadius:8, overflow:"hidden", flexShrink:0, background:"#eee" }}>
                {p.image ? <img src={p.image} style={{ width:"100%", height:"100%", objectFit:"cover" }}/> : <div style={{ width:"100%", height:"100%", background:"#ddd" }}/>}
              </div>
        ...
        return (
          <div key={cat.id} style={{ borderRadius:"12px", border:`1px solid ${sel?"#444":"#2a2a2a"}`, background:sel?"rgba(255,255,255,0.04)":"#1a1a1a", overflow:"hidden" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"10px", padding:"10px 12px", cursor:"pointer" }} onClick={()=>toggle(cat)}>
              <div style={{ width:38, height:38, borderRadius:"50%", overflow:"hidden", flexShrink:0, background:"#eee", display:"flex", alignItems:"center", justifyContent:"center" }}>
            ...
            {sel && (
              <div style={{ padding:"0 12px 10px" }}>
                ...
                <button onClick={()=>imgRef.current[cat.id]?.click()} style={{ width:"100%", padding:"6px", borderRadius:"8px", background:"#eee", border:"1px dashed #333", color:"#555", fontSize:"11px", cursor:"pointer", ...F }}>
...
function Editor({ section, onSave, onClose, products, cats }: { section:Section; onSave:(cfg:Record<string,any>)=>void; onClose:()=>void; products:Product[]; cats:Category[] }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", backdropFilter:"blur(6px)", zIndex:600, display:"flex", alignItems:"center", justifyContent:"center", padding:"16px" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"#fff", border:"1px solid #e5e5e5", borderRadius:"20px", width:"100%", maxWidth:"520px", maxHeight:"88vh", overflowY:"auto", direction:"rtl", ...F }}>
        <div style={{ padding:"16px 20px", borderBottom:"1px solid #1a1a1a", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, background:"#fff", zIndex:1 }}>
          <div>
          ...
          <button onClick={onClose} style={{ width:30, height:30, borderRadius:9, background:"#f0f0f0", border:"1px solid #ddd", color:"#888", cursor:"pointer", fontSize:14 }}>✕</button>
        </div>
        ...
        <div style={{ padding:"20px" }}>
          ...
          {section.type==="banner_dual" && (<>
            <div style={{ marginBottom:"14px", padding:"14px", background:"#f0f0f0", borderRadius:"12px", border:"1px solid #2a2a2a" }}>
            ...
            <div style={{ padding:"14px", background:"#f0f0f0", borderRadius:"12px", border:"1px solid #2a2a2a" }}>
          ...
          {section.type==="banner_triple" && (<>
            {[1,2,3].map(n=>(
              <div key={n} style={{ marginBottom:"12px", padding:"12px", background:"#f0f0f0", borderRadius:"12px", border:"1px solid #2a2a2a" }}>
        ...
        <div style={{ padding:"14px 20px", borderTop:"1px solid #1a1a1a", display:"flex", gap:"8px", justifyContent:"flex-end", position:"sticky", bottom:0, background:"#fff" }}>
          <button onClick={onClose} style={{ background:"#f0f0f0", border:"1px solid #ddd", color:"#888", padding:"9px 18px", borderRadius:"10px", fontSize:"13px", cursor:"pointer", ...F }}>إلغاء</button>
...
export default function BrandBuilderPage() {
    ...
    const load = async () => {
      try {
        const [bRes, pRes, cRes] = await Promise.all([
          fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/api/brands/${username}/builder`),
  ...
  const save = async () => {
    setSaving(true)
    try {
      await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/api/brands/${username}/builder`, {
        method:"PUT",
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#EEEEEE", color:"white", direction:"rtl", ...F }}>
      <div style={{ padding:"20px", maxWidth:"680px", margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"20px" }}>
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
              ...
              <span style={{ fontSize:"12px", color:"#555", background:"#fff", border:"1px solid #e5e5e5", padding:"2px 10px", borderRadius:"20px" }}>@{username}</span>
            </div>
            <p style={{ fontSize:"11px", color:"#444", marginTop:"4px" }}>اسحب لتغيير الترتيب — اضغط الإعدادات لتعديل المحتوى</p>
          </div>
          <div style={{ display:"flex", gap:"8px" }}>
            <button onClick={()=>setShowAdd(true)} style={{ background:"#f0f0f0", border:"1px solid #ddd", color:"#111", padding:"8px 16px", borderRadius:"10px", fontSize:"13px", fontWeight:700, cursor:"pointer", ...F }}>+ قسم</button>
        ...
        <a href={`https://klafstore.com/brand/${username}`} target="_blank" rel="noopener noreferrer"
          style={{ display:"flex", alignItems:"center", gap:"8px", padding:"10px 14px", borderRadius:"12px", background:"#f8f8f8", border:"1px solid #e5e5e5", color:"#555", fontSize:"12px", textDecoration:"none", marginBottom:"16px" }}>
            ...
            {sections.map((sec, idx) => (
              <div key={sec.id} draggable onDragStart={()=>onDragStart(idx)} onDragOver={e=>onDragOver(e,idx)} onDragEnd={onDragEnd}
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"5px", flexShrink:0 }}>
                  <button onClick={()=>setEditSec(sec)} style={{ width:30, height:30, borderRadius:8, background:"#f0f0f0", border:"1px solid #2a2a2a", color:"#888", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  ...
                  <button onClick={()=>moveUp(idx)} disabled={idx===0} style={{ width:30, height:30, borderRadius:8, background:"#f0f0f0", border:"1px solid #2a2a2a", color:idx===0?"#2a2a2a":"#888", cursor:idx===0?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  ...
                  <button onClick={()=>moveDown(idx)} disabled={idx===sections.length-1} style={{ width:30, height:30, borderRadius:8, background:"#f0f0f0", border:"1px solid #2a2a2a", color:idx===sections.length-1?"#2a2a2a":"#888", cursor:idx===sections.length-1?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      {showAdd && (
        <div onClick={()=>setShowAdd(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", backdropFilter:"blur(6px)", zIndex:600, display:"flex", alignItems:"center", justifyContent:"center", padding:"16px" }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"#fff", border:"1px solid #e5e5e5", borderRadius:"20px", width:"100%", maxWidth:"480px", maxHeight:"85vh", overflowY:"auto", direction:"rtl", ...F }}>
            <div style={{ padding:"16px 20px", borderBottom:"1px solid #1a1a1a", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <span style={{ fontSize:"15px", fontWeight:800 }}>إضافة قسم</span>
              <button onClick={()=>setShowAdd(false)} style={{ width:30, height:30, borderRadius:9, background:"#f0f0f0", border:"1px solid #ddd", color:"#888", cursor:"pointer", fontSize:14 }}>✕</button>
            </div>
            <div style={{ padding:"16px", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px" }}>
              {(Object.entries(META) as [SectionType,any][]).map(([type,meta])=>(
                <div key={type} onClick={()=>addSec(type)}
                  style={{ padding:"13px", borderRadius:"12px", cursor:"pointer", background:"#f0f0f0", border:"1px solid #2a2a2a" }}
