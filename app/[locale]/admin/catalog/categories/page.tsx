...
export default function CategoriesPage() {
  ...
  const save = async () => {
    if (!name.trim()) return
    setSaving(true)
    const body = JSON.stringify({ name, name_en: nameEn || null, parent_id: parentId })
    if (editCat) {
      await fetch("/api/admin/categories/" + editCat.id, { method: "PUT", headers: { "Content-Type": "application/json" }, body })
    } else {
      await fetch("/api/admin/categories", { method: "POST", headers: { "Content-Type": "application/json" }, body })
    }
    await loadCats()
    setName(""); setNameEn(""); setParentId(null); setEditCat(null); setShowAdd(false); setSaving(false); setLang("ar")
  }
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#EEEEEE",color:"#111",fontFamily:"Cairo,system-ui,sans-serif",direction:"rtl" }}>
      <div style={{ padding:"16px 20px",borderBottom:"1px solid #e5e5e5",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,background:"#fff",zIndex:10 }}>
        ...
        <button onClick={()=>{setShowAdd(true);setEditCat(null);setName("");setNameEn("");setParentId(null);setLang("ar")}} style={{ fontSize:"12px",background:"#fff",color:"#000",padding:"6px 14px",borderRadius:"8px",fontWeight:"700",border:"none",cursor:"pointer",fontFamily:"Cairo,system-ui,sans-serif" }}>+ جديد</button>
      </div>
      ...
      <div style={{ padding:"16px",maxWidth:"700px",margin:"0 auto" }}>
        ...
        {loading ? <p style={{ color:"#555",textAlign:"center",padding:"40px 0" }}>جاري التحميل...</p> : (
          <div style={{ display:"flex",flexDirection:"column",gap:"8px" }}>
            {parents.filter(c=>search===""||c.name.includes(search)||(c.name_en||"").toLowerCase().includes(search.toLowerCase())).map(parent => (
              <div key={parent.id}>
                <div style={{ background:"#fff",border:"1px solid #e5e5e5",borderRadius:"12px",padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px" }}>
                  <div style={{ display:"flex",alignItems:"center",gap:"10px" }}>
                    ...
                    <div>
                      <span style={{ fontWeight:"700",fontSize:"14px",display:"block" }}>{parent.name}</span>
                      {parent.name_en && <span style={{ fontSize:"11px",color:"#888" }}>EN: {parent.name_en}</span>}
                    </div>
                    <span style={{ color:"#555",fontSize:"11px",marginRight:"8px" }}>{cats.filter(c=>c.parent_id===parent.id).length} فرعي</span>
                  </div>
                  <div style={{ display:"flex",gap:"6px" }}>
                    ...
                    <button onClick={()=>deleteCat(parent.id)} style={{ background:"#fff0f0",border:"1px solid #fed7d7",color:"#dc2626",padding:"5px 12px",borderRadius:"7px",fontSize:"11px",fontWeight:"700",cursor:"pointer",fontFamily:"Cairo,system-ui,sans-serif" }}>حذف</button>
                  </div>
                </div>
                {cats.filter(c=>c.parent_id===parent.id&&(search===""||c.name.includes(search)||(c.name_en||"").toLowerCase().includes(search.toLowerCase()))).map(child => (
                  <div key={child.id} style={{ background:"#f8f8f8",border:"1px solid #e5e5e5",borderRadius:"10px",padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px",marginRight:"16px" }}>
                    <div style={{ display:"flex",alignItems:"center",gap:"10px" }}>
                      ...
                      <div>
                        <span style={{ fontSize:"13px",color:"#444",display:"block" }}>{child.name}</span>
                        {child.name_en && <span style={{ fontSize:"10px",color:"#aaa" }}>EN: {child.name_en}</span>}
                      </div>
                    </div>
                    <div style={{ display:"flex",gap:"6px" }}>
                      ...
                      <button onClick={()=>deleteCat(child.id)} style={{ background:"#fff0f0",border:"1px solid #fed7d7",color:"#dc2626",padding:"4px 10px",borderRadius:"7px",fontSize:"11px",fontWeight:"700",cursor:"pointer",fontFamily:"Cairo,system-ui,sans-serif" }}>حذف</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showAdd && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={()=>setShowAdd(false)}>
          <div style={{ background:"#fff",borderRadius:"20px 20px 0 0",padding:"20px",width:"100%",maxWidth:"500px" }} onClick={e=>e.stopPropagation()}>
            ...
            {/* Language Switcher */}
            <div style={{ display:"flex",gap:0,marginBottom:14,background:"#f5f5f5",padding:4,borderRadius:10,width:"fit-content" }}>
              <button onClick={()=>setLang("ar")} style={{ padding:"7px 16px",background:lang==="ar"?"#fff":"transparent",border:"none",borderRadius:7,cursor:"pointer",fontWeight:800,fontSize:12,fontFamily:"inherit",color:lang==="ar"?"#FF835E":"#666" }}>
                🇸🇦 العربية
              </button>
              <button onClick={()=>setLang("en")} style={{ padding:"7px 16px",background:lang==="en"?"#fff":"transparent",border:"none",borderRadius:7,cursor:"pointer",fontWeight:800,fontSize:12,fontFamily:"inherit",color:lang==="en"?"#FF835E":"#666" }}>
                🇬🇧 English
              </button>
            </div>

            {lang === "ar" ? (
              <input value={name} onChange={e=>setName(e.target.value)} placeholder="اسم القسم (عربي)" style={inp} dir="rtl" />
            ) : (
              <input value={nameEn} onChange={e=>setNameEn(e.target.value)} placeholder="Category name (English)" style={inp} dir="ltr" />
            )}
            ...
            <button onClick={save} disabled={saving||!name.trim()} style={{ width:"100%",padding:"13px",background:saving||!name.trim()?"#ccc":"#FF835E",color:"#fff",border:"none",borderRadius:"12px",fontWeight:"800",fontSize:"14px",cursor:saving||!name.trim()?"not-allowed":"pointer",fontFamily:"Cairo,system-ui,sans-serif" }}>
