...
async function getUsers(token: string) {
  try {
    const res = await fetch("${process.env.NEXT_PUBLIC_API_URL}/api/users/trash", {
...
function UserCard({ user }: { user: any }) {
  return (
    <div style={{ background: "#fff", border: "1px solid #e5e5e5", borderRadius: "12px", padding: "12px 14px", display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
      <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "800", fontSize: "14px", flexShrink: 0 }}>
        {user.name?.charAt(0) || "؟"}
      </div>
