...
async function getAffiliates(token: string) {
  try {
    const res = await fetch("${process.env.NEXT_PUBLIC_API_URL}/api/affiliate/stats", {
        ...
        {affiliates.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {affiliates.map((a: any) => (
              <div key={a.id} style={{ background: "#fff", border: "1px solid #e5e5e5", borderRadius: "14px", padding: "14px 16px" }}>
