...
export default function AdsPage() {
  ...
  const toggleActive = async (ad: any) => {
    await fetch(`${apiUrl}/api/ads/${ad.id}`, { method:"PUT", headers:{ Authorization:"Bearer "+getToken(), "Content-Type":"application/json" }, body: JSON.stringify({ is_active: ad.is_active ? 0 : 1 }) })
    fetchAds()
  }
  ...
  const IconDelete = () => <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>
        ...
        <div style={{ background:"#fff", borderRadius:"14px", padding:"20px", marginBottom:20, border:"1px solid #e5e5e5" }}>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <div
              ...
              style={{ border:"2px dashed #e5e5e5", borderRadius:"12px", overflow:"hidden", cursor: form.image_url ? "default" : "pointer", minHeight:120, display:"flex", alignItems:"center", justifyContent:"center", background:"#fafafa", position:"relative" as const }}
            >
              {form.image_url ? (
                <>
                  {form.media_type === "video"
                    ? <video src={form.image_url} style={{ width:"100%", maxHeight:180, display:"block" }} controls />
                    : <img src={form.image_url} style={{ width:"100%", maxHeight:180, objectFit:"cover", display:"block" }} alt="" />}
                  ...
                  <button onClick={e => { e.stopPropagation(); fileRef.current?.click() }} style={{ position:"absolute" as const, top:8, right:8, background:"rgba(0,0,0,.6)", color:"#fff", border:"none", borderRadius:"8px", padding:"4px 10px", cursor:"pointer", fontSize:11, fontWeight:"700", fontFamily:"Cairo,system-ui,sans-serif", display:"flex", alignItems:"center", gap:4 }}>
                    <IconUpload /> تغيير
                  </button>
                </>
              ) : (
                <div style={{ textAlign:"center" as const, padding:20 }}>
                  {uploading ? (
                    <div style={{ color:"#FF835E", fontSize:"13px", fontWeight:"700" }}>جاري الرفع...</div>
                  ) : (
                    <>
            ...
            {(form.title || form.image_url) && (
              <div style={{ marginTop:10 }}>
                ...
                <div style={{ borderRadius:"12px", overflow:"hidden", border:"1px solid #e5e5e5" }}>
                  {form.image_url ? (
                    form.media_type === "video"
                      ? <video src={form.image_url} style={{ width:"100%", maxHeight:100, display:"block" }} />
                      : <img src={form.image_url} style={{ width:"100%", maxHeight:100, objectFit:"cover", display:"block" }} alt="" />
                  ) : (
          ...
          {ads.map((ad:any) => (
            <div key={ad.id} style={{ background:"#fff", borderRadius:"14px", padding:"14px 16px", border:"1px solid #e5e5e5", display:"flex", alignItems:"center", gap:12 }}>
              ...
              <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                ...
                <button onClick={() => startEdit(ad)} style={{ background:"#f5f5f7", border:"none", borderRadius:"8px", padding:"6px 10px", cursor:"pointer", display:"flex", alignItems:"center" }}>
                  <IconEdit />
                </button>
                <button onClick={() => deleteAd(ad.id)} style={{ background:"#fef2f2", border:"none", borderRadius:"8px", padding:"6px 10px", cursor:"pointer", display:"flex", alignItems:"center" }}>
                  <IconDelete />
                </button>
              </div>
            </div>
