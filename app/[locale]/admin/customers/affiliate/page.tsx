...
export default async function AffiliatePage() {
  ...
  const affiliates = allUsers.filter((u: any) => u.role === "affiliate")
  ...
  return (
    <div style={{ minHeight: "100vh", background:"#EEEEEE", color: "#111", fontFamily: "Cairo, system-ui, sans-serif", direction: "rtl" }}>
      ...
      <div style={{ display: "flex", gap: "4px", padding: "12px 16px", borderBottom: "1px solid #e5e5e5", overflowX: "auto" }}>
        {tabs.map(tab => (
          <Link key={tab.href} href={tab.href} style={{ textDecoration: "none" }}>
            <div style={{ padding: "8px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: "600", whiteSpace: "nowrap", background: tab.role === "affiliate" ? "#fff" : "#111", color: tab.role === "affiliate" ? "#000" : "#555", border: "1px solid #e5e5e5" }}>
              {tab.label}
            </div>
          </Link>
        ))}
      ...
      <div style={{ padding: "16px", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "12px" }}>
        ...
        {affiliates.length === 0 && (
          <p style={{ color: "#555", textAlign: "center", padding: "40px", gridColumn: "1/-1" }}>لا يوجد شركاء عمولة</p>
        )}
      </div>
    </div>
  )
